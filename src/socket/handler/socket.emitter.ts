...
import {first, map, timeout} from "rxjs/internal/operators";
...
export class SocketEventEmitterListener<E,L> extends SocketEventListener<L>{
  ...
  listenWithCallback(): Observable<SocketDataWrapper<L, E>>  {
    return this.listen().pipe(map((args) => {
      return [
        args[0] || args,
        args[1] || (() => console.warn(`No callback received from event ${this.eventName}`))
      ]
    } ))
  }
...
export type SocketDataWrapper<L,E> = [ L,  (resp: E | string) => void]
